...
class CQADupStackEvaluator(BaseEvaluator, CQADupStackData):
    ...
    def __init__(self, semb_fn, datasets_dir='datasets/usep/data-eval/cqadupstack', forum='all', bsz=32, show=True):
    ...
    def _run(self, eval_type):
        ...
        for forum in self.dnames:
            ...
            qembs = self._text2se(queries, normalize=True)
            ...
            for did, doc in pool.items():
                dids.append(did)
                docs.append(doc)
            dembs = self._text2se(docs, normalize=True)
            ...
            if self.show: logger.info(f'map/ndcg for {forum}: {mean_ap_score}/{mean_ndcg_score}')
        ...
        if len(self.dnames) > 1:
            ...
            if self.show: logger.info(f'map/ndcg for average: {map_avg}/{ndcg_avg}')
        return results
