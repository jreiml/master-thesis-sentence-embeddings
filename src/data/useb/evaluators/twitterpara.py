...
class TwitterParaEvaluator(BaseEvaluator):
    ...
    def __init__(self, semb_fn, dataset_dir='datasets/usep/data-eval/twitterpara', dname='all', bsz=32, show=True):
    ...
    def _run(self, eval_type=None):
        if eval_type == 'valid':
            logger.warning('TwitterPara does not have a development set and here the model is evaluated on the test set instead.')
        results = {}
        for dname in self.dnames:
            ...
            s1_embs = self._text2se(s1s)  # (bsz, hdim)
            s2_embs = self._text2se(s2s)  # (bsz, hdim)
